...
func define_power():
	...
	for mx in slotManager.all_friendly_cards() if is_friendly else slotManager.all_enemy_cards():
		if "sigils" in mx.card_data and "Purple Mox" in mx.card_data["sigils"]:
			attack += 1
		if "sigils" in mx.card_data and "Purple Mox (2)" in mx.card_data["sigils"]:
			attack += 2
		if "sigils" in mx.card_data and "Purple Mox (3)" in mx.card_data["sigils"]:
			attack += 3
		if "sigils" in mx.card_data and "Purple Mox (4)" in mx.card_data["sigils"]:
			attack += 4
	return attack
